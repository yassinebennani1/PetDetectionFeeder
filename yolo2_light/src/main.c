...
int my_fetch()
{
    in = get_image_from_stream_resize_cpu(cap, net.w, net.h, &in_img);
    if (!in.data) {
        error("Stream closed.");
    }
...
{
  // initialize global pet detection array to -1s
    int w;
    for (w = 0; w < 8; w++) {
      pet_detect[w] = -1;
    }
    int fd;
    // Open port for read/write, no "controlling tty" status,
    // and regardless what state DCD is in 
    fd = open("/dev/ttyACM0", O_RDWR | O_NOCTTY | O_NDELAY);
    if (fd == -1) {
        perror("open_port: Unable to open /dev/ttyACM0 - ");
        return -1;
    }
    // TODO: turn on blocking?
    fcntl(fd, F_SETFL, 0);
    // Linux Raw serial setup options
    struct termios options;
    tcgetattr(fd,&options); // Get current serial settings in structure
    cfsetspeed(&options, B9600); // Change only a few
	...
	if (delay == 0) {
	  ...
	  disp = det;
	  show_img = det_img;
	  save_image_png(disp, "last");
	}

	// loop through detections of interest
	for (i = 0; i < 8; i++) {
            if (pet_detect[i] == 0) {
                //n = write(fd,((pet_detect[i] % 14) + '0'),1);
                printf("PERSON\n");
                n = write(fd,"0",1);
            } else if (pet_detect[i] == 16) {
                printf("DOG\n");
                n = write(fd, "1", 1);
            } else if (pet_detect[i] == 15) {
                printf("CAT\n");
                n = write(fd, "2", 1);
            }
            if (n < 0) {
